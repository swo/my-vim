...
def create_symlink(dst, src, debug=False):
    ...
    if exists and not islink:
        ...
        if actual_src == src:
            ...
            raise RuntimeError(f"{dst} is symlinked to {actual_src}, not {src}. Delete it.")
    elif not exists:
...
package_dir_parent = os.path.expanduser(f"~/.vim/pack/plugins")
...
create_symlink(os.path.expanduser(f"~/.vim/pack/plugins/start"), os.path.abspath("start"))
